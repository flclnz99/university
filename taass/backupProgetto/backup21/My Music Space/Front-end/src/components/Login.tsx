...
function Login({ setUserProfile, setIsLogged }) {
  ...
  function checkInputAuth() {
    ...
    if (!regex.test(email)) {
      swal(
        "Login",
        "Email non corretta. Inserisci una mail valida!",
        "warning"
      );
      return false;
    }
        ...
        success: function (response) {
          if (response.response == "User not found.") {
            ...
            let userProfile = {
              ...
              email: myArray[0].trim(),
              name: myArray[1].trim(),
              surname: myArray[2].trim(),
            };
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.response == "Error: username already exist") {
            swal(
              "Registrazione",
              "Email esistente. Perfavore inserisci un'altra mail",
              "error"
            );
          } else {
    ...
    <div className="Login" id="login">
      ...
      <div id="registrazionePopup">
        <div id="registrazionePopupContent">
          ...
          <span className="closeButton" onClick={() => chiudiPopup()}>
            &#x2717;
          </span>
          <div>
