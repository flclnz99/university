...
public class Client {
    ...
    private final StringProperty newMessage;
    Socket socket = null;
    ...
    public Property<String> newMessage() {return newMessage;}

    /**
     * @return elimina l'email specificata
     */
    public void deleteEmail(Email email, String host, int port) {
        ...
        request.setClient(emailAddressProperty().getValue());
    ...
    private boolean tryCommunication(String host, int port) throws IOException {
        try {
            ...
            System.out.println("sono "+ emailAddress +", mando richiesta con index = "+ request.getIndex());
           /* Integer index = inboxContent.size();/*è forse meglio una variabile globale?
            String user = emailAddressProperty().getValue(); /*###mandarla come property? Come?###*/
            sendRequest(request);
           // sendIndex(index);
            //sendAddress(user);
            receiveData(request.getIndex());
 ...
 /*   private void sendIndex(Integer index) throws IOException {
    ...
    private void receiveData(int index) throws IOException, ClassNotFoundException, ParseException {
        ArrayList<JSONValue> myData = (ArrayList<JSONValue>) inputStream.readObject();
        if(!myData.isEmpty()&&index!=0){
            Platform.runLater(() -> newMessage.set("new message"));
        } else {
            Platform.runLater(() -> newMessage.set(""));
        }
    ...
    public void sendEmail(Email email, String host, int port) {
        try {
            connectToServer(host, port);
        } catch (IOException e) {
            e.printStackTrace();
